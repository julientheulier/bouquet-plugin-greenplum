...
import com.squid.core.jdbc.vendor.greenplum.postgresql.PostgresqlStatistics;

/**
 * extend postgres support:
 * - handles min/max statistics using histogram
 * - handles partition info
 * 
 * @author sergefantino
 *
 */
public class GreenplumStatistics extends PostgresqlStatistics {

    static final Logger logger = LoggerFactory.getLogger(GreenplumStatistics.class);
    
    private DataSourceReliable ds;

    public GreenplumStatistics(DataSourceReliable ds) {
        super(ds);
        this.ds = ds;
    }
    ...
    protected void computeColumnsStatistics(Table table, Connection connection) {
        try {
            ...
            try {
                ...
                String sql = "select attname, n_distinct, histogram_bounds from pg_stats where schemaname='"+table.getSchema().getName()+"' and tablename='"+table.getName()+"'";
                ...
                while (resultset.next()) {
                    ...
                    if (col!=null) {
                        ...
                        if (histogram==null) {
                            ...
                            try {
                                int x = histogram.getBaseType();
                                Object[] data = (Object[]) histogram.getArray();
                                Object min = data.length>1?data[0]:null;
                                Object max = data.length>1?data[1]:null;
                                putStatistics(col,new ColumnStatistics(stats,min,max));
                            } catch (Exception e) {
                                putStatistics(col,new ColumnStatistics(stats));
                            }
                        }
    ...
    /**
     * return partition information for the given table; if the table is not partitioned, partitionInfo.hasPartition() return false;
     * @param table
     * @return
     */
    ...
    private void populatePartitionTables(Statement statement, PartitionInfo info) throws SQLException, ExecutionException {
        Table table = info.getTable();
        String sql = "select partitiontablename, partitionrangestart, partitionrangeend from pg_partitions WHERE schemaname='"+table.getSchema().getName()+"' and tablename='"+table.getName()+"' order by partitionrangestart";
    ...
    protected void populatePartitionKeys(Statement statement, PartitionInfo info) throws SQLException, ExecutionException {
        Table table = info.getTable();
        String sql = "select columnname from pg_partition_columns WHERE schemaname='"+table.getSchema().getName()+"' and tablename='"+table.getName()+"' order by position_in_partition_key ASC";
        ResultSet resultset = statement.executeQuery(sql);
        while (resultset.next()) {
